...
#define EVAL ((clock=!clock),main_time+=1,this->eval(main_time)) //eval one cycle, return 1 if exit
	...
	ram->read_random_vlog();
        ...
        printf("Start\n");
        //计时开始
        auto nemu_start = std::chrono::steady_clock::now();
        while(true){
            // Simulate until exit
                if (main_time % 100000 == 1)
                {
                    //计算时间，频率=main_time/到现在的时间
                    auto nemu_end = std::chrono::steady_clock::now();
                    timer = std::chrono::duration_cast<std::chrono::nanoseconds>(nemu_end - nemu_start).count();//单位是ns
                    //ns->ms
